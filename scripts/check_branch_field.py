"""Check Branch SingleSelect options in Transaction History."""
import json
...
for f in r["data"]["items"]:
    if f["field_name"] == "Branch":
        print(f"Branch field id: {f['field_id']}")
        print(f"Property: {json.dumps(f.get('property'), indent=2)}")
        break
